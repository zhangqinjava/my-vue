...
import {getMenu} from '@/api/systemApi'
...
router.beforeEach(async (to, from, next) => {
    ...
    if (to.meta.title) {
        ...
        document.title = "AssetManagement"
    }

    // 动态添加路由
    if(sessionStorage.getItem('token') && flag){
        const menuStore = useMenuStore();
        await getMenu({type:'0', status: 1}).then((res)=>{
            let result = routerFilter(res.data)
...
export function routerFilter(data) {
    data.forEach((item)=>{
        ...
        if(item.parentId == '0'){
            ...
            item.icon = item.icon;
            item.meta = { title: item.chineseName };
            console.log(modules[`/src/views${item.url}.vue`])
            console.log(item.url,"======")
            item.component = modules[`/src/views${item.url}.vue`];
        }
