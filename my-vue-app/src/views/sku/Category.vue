    ...
    <el-dialog   v-model="state.addVisit" title="添加商品分类" :close-on-click-model="false" width="30%">
        ...
        <template #footer>
                <span class="dialog-footer">
                    <el-button type="primary"  @click="onSubmit" icon="Check" :loading="state.loading">提交</el-button>
...
import {categorySearch,categorySave,categoryUpdate,categoryDelete,categoryList,exportTree} from '@/api/skuApi.js';
...
const onSearch=()=>{
    state.loading=true;
    categorySearch(Object.assign(state.searchForm,state.page)).then((res)=>{
        if(res.code==200){
            state.loading=false;
...
const onSubmit=async ()=>{
    ...
    await form.validate();
    state.loading=true;
    categorySave(Object.assign(state.addForm)).then((res)=>{
        if(res.code==200){
...
const remoteMethod = (query) => {
    state.loading = true
    try {
     categoryList(Object.assign(state.searchForm)).then((res)=>{
        if(res.code=200){
...
const editSubbit=async()=>{
    ...
    categoryUpdate(Object.assign({},state.editForm)).then((res)=>{
        if(res.code==200){
