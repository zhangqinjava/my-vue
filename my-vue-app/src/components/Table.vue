        ...
        <div class="table-wrap">
            <el-table :data="data" border stripe size="mini" :height="tableHeight" :max-height="maxHeight" row-key="id" :tree-props="{children: 'children', hasChildren: 'hasChildren'}"
                ...
                <template v-for="item in column" :key="item">
                    ...
                    <!-- DXD 下拉元素匹配类型 -->
                    <el-table-column v-else-if="item.type == 'optionText'" show-overflow-tooltip 
                        :label="item.label" :align="item.align != null ? item.align : 'center'" :width="item.width">
                        <template #default="scope">
                            <span v-for="itemOne in item.options" :key="itemOne.value" >
                                <text v-if="scope.row[item.prop] === itemOne.value" >
                                    {{ itemOne.label }}
                                </text>
                            </span> 
                        </template>
                    </el-table-column>
                    <!-- DXD 展开行元素类型 注意内敛的数据命名必须为 rateTdVoList-->
                    <el-table-column v-else-if="item.type == 'expand'" show-overflow-tooltip type="expand"
                        :label="item.label" :align="item.align != null ? item.align : 'center'" :width="item.width">
                        <template  #default="scope">
                            <el-table :data="scope.row.rateTdVoList" border
                                 style="width: 55%"    
                                 :header-cell-style="{
                                    background: '#e6eaf2',
                                    color: '#666',
                                 ...
                                 max-height="590">
                                <el-table-column v-for="one in item.data" key="one"
                                    :prop="one.prop"
                                    :label="one.label"
